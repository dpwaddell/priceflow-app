...
const BACKUP_DIR = path.join(
  APP_ROOT,
  "backups",
  `${new Date().toISOString().replace(/[:.]/g, "-")}_phase6_hide_customer_search`
);
...
try {
  ...
  try {
    ...
    execSync('git commit -m "Hide Shopify customer search until protected customer data is approved"', {
      cwd: APP_ROOT,
      env: process.env,
      shell: "/bin/bash",
      stdio: "inherit",
  ...
  log("");
  log("Done.");
  log("Customer assignments will now show a clear manual-entry message instead of a blocked search flow.");
} catch (err) {
  console.error("");
