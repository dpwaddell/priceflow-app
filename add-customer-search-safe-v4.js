...
const BACKUP_DIR = path.join(
  APP_ROOT,
  "backups",
  `${new Date().toISOString().replace(/[:.]/g, "-")}_phase6_customer_search_safe_v4`
);
...
try {
  ...
  if (!txt.includes('app.post("/customer-assignments", async (req, res) => {')) {
    fail('Could not find POST /customer-assignments route');
  }
...
`;
  ...
  const rendererBlock = findBlockByBrace(txt, "function renderCustomerAssignmentsPage(");
  txt = txt.slice(0, rendererBlock.start) + newRenderer + "\n\n" + txt.slice(rendererBlock.end);
  ...
  const getRouteBlock = findBlockByBrace(txt, 'app.get("/customer-assignments", async (req, res) => {');
  txt = txt.slice(0, getRouteBlock.start) + newGetRoute + "\n\n" + txt.slice(getRouteBlock.end);
