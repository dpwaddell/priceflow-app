...
const BACKUP_DIR = path.join(
  APP_ROOT,
  "backups",
  `${new Date().toISOString().replace(/[:.]/g, "-")}_phase6_customer_search_safe`
);
...
function replaceBlock(txt, startMarker, endMarker, replacement, label) {
  const start = txt.indexOf(startMarker);
  if (start === -1) fail(`Could not find start marker for ${label}`);
  const end = txt.indexOf(endMarker, start);
  if (end === -1) fail(`Could not find end marker for ${label}`);
...
try {
  ...
  if (!txt.includes('async function getCustomerAssignments(shopId)')) {
    fail("Current server.js does not look like the working customer assignments build");
  }
  ...
  if (!txt.includes("async function shopifyAdminGraphQL(")) {
    const helperInsertPoint = 'app.use(express.urlencoded({ extended: true }));';
...
async function searchShopifyCustomers(shopDomain, term) {
  ...
  return Array.from(seen.values());
    ...
    txt = txt.replace(helperInsertPoint, helperBlock);
  }
...
`;
  ...
  txt = replaceBlock(
    txt,
    "function renderCustomerAssignmentsPage(",
    '\napp.get("/health", async (req, res) => {',
    newRenderer + 'app.get("/health", async (req, res) => {',
    "renderCustomerAssignmentsPage"
  );

  const oldRoute = `app.get("/customer-assignments", async (req, res) => {
  try {
  ...
  const newRoute = `app.get("/customer-assignments", async (req, res) => {
  try {
    const shop = sanitizeShop(req.query.shop);
    const host = String(req.query.host || "");
  ...
  if (!txt.includes(oldRoute)) {
    fail("Could not find exact customer assignments route block to replace");
  }

  txt = txt.replace(oldRoute, newRoute);
  ...
  log("");
  log("Route markers:");
  run(`grep -n "searchShopifyCustomers\\|Find Shopify customer\\|Assign customer to tier" ${SERVER_FILE}`, { stdio: "inherit", shell: "/bin/bash" });
